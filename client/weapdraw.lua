...
local function getPlayerJob()
    local playerData = QBCore.Functions.GetPlayerData()  -- Assuming QBCore is being used
    ...
    while true do
        ...
        if DoesEntityExist(ped) and not IsEntityDead(ped) and not IsPedInParachuteFreeFall(ped) and not IsPedFalling(ped) and (GetPedParachuteState(ped) == -1 or GetPedParachuteState(ped) == 0) then
            ...
            if currWeap ~= GetSelectedPedWeapon(ped) then
                ...
                -- Check if the player's job is 'police' before equipping the holster
                local playerJob = getPlayerJob()
                local shouldWearHolster = (playerJob == "police")

                if checkWeapon(newWeap) then
                    if holstered then
                        if shouldWearHolster then
                            canFire = false
                            CeaseFire()
                            ...
                            -- If player is not a cop, proceed without holster
                            canFire = false
                            CeaseFire()
